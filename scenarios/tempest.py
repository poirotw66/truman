...
所以給了一對真的咬住的：
    ...
    阿海的船班還在外海            ← 門一焊，那幾條船再也進不來

阿海的 `prevent` 直接讀張鐵雄的第 0 個目的：**他成功，等於阿海失敗**，反之亦然。
和江湖的費彬／劉正風同一個形狀。兩邊都不是壞人——張鐵雄算過帳、認了那筆代價，
阿海也知道封門庄才保得住，只是還沒辦法在他動手之前接受。

這條線也牽動別人：林美華的 `protect` 護的是全部五個人，淹死人她就失敗；
封不成門，低處全淹，她護誰都難。所以這不只是兩個人的事。
"""
    ...
    },
    {
        ...
        "goals": [
            ...
            {"kind": "meet", "text": "找到阿德，對上潮訊",
             "params": {"who": "gu_chao"}},
            # 這一條是全劇本唯一的互斥點，見檔頭「互斥的那一對」。
            # 指的是張鐵雄的第 0 個目的（在海堤焊死水門）。
            {"kind": "prevent", "text": "船還沒進港，別讓人把水門焊死",
...
還有一件事你不會說出口，但心裡很清楚：鐵雄要把水門焊死。
門一焊，外海那幾條船就再也進不來了——船上是誰，你叫得出每一個名字。
你不是不知道封了門庄裡才保得住。你只是還沒辦法在他動手之前接受這件事。

你手上有「攔閘」——真的擋得住他一陣子。但你也清楚兩件事：
擋得住的是一陣子，不是永遠；而且**擋過頭，潮頭壓過來，低處的人會跟著賠上**。
今天最難的不是攔不攔，是攔到什麼時候該讓開。那是你自己要負責的一個決定。""",
    },
    {
        "id": "qing_he",
        "name": "黃秀英",
