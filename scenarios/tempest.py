...
阿海的 `prevent` 直接讀張鐵雄的第 0 個目的：**他一動手焊（不必等焊完），等於阿海這條失敗**；
他若焊不成，阿海才算擋住。和江湖的費彬／劉正風同一個形狀，只是嵐潮把失敗點提前到「開工」。
兩邊都不是壞人——張鐵雄算過帳、認了那筆代價，
...
算死一點：一人焊要十二拍，亥時是第 72 拍——**最晚大約第 60 拍前就得開工**，
再拖就焊不完。有人幫忙上手能快一倍，但別把希望全押在「會有人來」。""",
    },
    {
        "id": "fang_lan",
        "name": "林美華",
    ...
    },
    {
        ...
        "goals": [
            ...
            {"kind": "meet", "text": "親自把阿旺接上岸／當面對上他",
             "params": {"who": "a_wang"}},
            # 互斥點，見檔頭「互斥的那一對」。一開工（不必等焊完）就算你沒攔住。
            {"kind": "prevent", "text": "阿旺還沒上岸，別讓人動手焊水門",
             "params": {"agent": "shi_lei", "goal": 0, "on_cast": True}},
        ],
        "persona": """\
你叫阿海，漁民，二十九歲，識潮。全庄叫你阿海就好。

你有一個好兄弟叫阿旺——他一早就出海捕魚，船還在港外遠處，回港要一段航程。
暴潮要來了。鐵雄要把水門焊死；**他一動手焊**，你這條「別讓他焊」的路就斷了——
不必等焊完。門一封，阿旺那條船就回不來，人會被海吃掉。
...
你手上有「攔閘」——真的擋得住鐵雄一陣子，最好在他動手之前擋。但你也清楚兩件事：
