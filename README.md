...
**補上一對真的咬住的，重跑之後結果變了**（`張鐵雄 ←→ 阿海`）：水門焊死庄才保得住，
但門一焊，阿海還在外海的船就再也進不來。阿海的 `prevent` 直接讀張鐵雄的目的。

| | seed 42 | seed 7 |
|---|---|---|
| 舊設定 `t1`／`t2` | held | held（結局文字逐字相同）|
| 新設定 `t5`／`t6` | **partial** | **lost**（四人溺斃）|

12 條目的裡 **4 條分岔**（前一組是 0 條）。沒有人寫這兩個結局。

**但真正的教訓不是「互斥有效」**——`t3`／`t4` 有同一對互斥卻毫無產出，因為水閘第 8／15 拍
就焊完了，而期限在 72。**互斥＋窗口才生得出分岔；互斥單獨只是一場戲。**
給水閘加了時間門檻（`not_before`）與可以擋它的手段（`block_rite`）之後，
焊門才被推到 t44，那個窗口才真的存在。見 `todo.md` 第 1 條。
