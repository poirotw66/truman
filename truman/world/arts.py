...
## 效果一覽（引擎怎麼解，見 Engine._resolve_art）
    ...
    rite      當眾完成一場儀式（需要有人在場觀禮；可設 not_before 時間門檻；
              可設 duration>1，開工後要連續做完才算數，中途可被打斷）
    block_rite  擋下某個儀式一段時間——反對者手上真正的籌碼；若對方正在施工，一併打斷
"""
...
CATALOG: dict[str, ArtDef] = {
    ...
    "zhen_chao_li": ArtDef(
        ...
        name="做海醮",
        kind="social",
        tagline="在鎮廟當眾起香做醮：至少兩名觀禮者從頭見證到尾。禮要做完一段時間，不是敲一下鐘就成。",
        when="潮快上岸、鎮廟裡至少有兩名觀禮者正在你可見範圍內看著你的時候。這是今天唯一能擋滅村的大禮。"
        "觀禮者不足就先別開禮；潮頭一過再做也晚了。"
        "**起醮之後要連續做完一段時間**，中途觀禮者走光或你離開鎮廟，這場就不算。",
        effect="rite",
        params={"area": "鎮廟", "witnesses": 2, "rite": "做海醮", "duration": 8,
                "keep_witnesses": True},
        uses=1,
    ),
    "feng_zha": ArtDef(
        ...
        name="焊水門",
        kind="social",
        tagline="把海堤水門焊死。一人焊大約十二刻；再多一個人上手幫忙，能快一倍。",
        when="**午時前後、潮訊確認、外海那道白線推近之後**，"
        "你人在海堤、旁邊還有人能作證的時候。"
        "太早焊不得——水門是全庄進出海的口子，浪還沒起就封死，等於自己斷了漁港，"
        "沒回港的船也一條都回不來。但焊晚了，醮也救不了低處。"
        "**這是苦力活：一個人焊大約要十二刻才封得死**；海堤上若再有一個人上手幫忙，進度快一倍。"
        "亥時約第 72 拍——**一人焊的話，最晚大約第 60 拍前就得開工**，再拖就焊不完。"
        "作證的人全程都得在你看得見的地方——走光了就得重來。"
        "焊的時候你走不開；被人攔下、或自己離開海堤，也得重來。"
        "門一焊死，外海回港的路就斷了。什麼時候動手，是你今天最難的一個決定。",
        effect="rite",
        # duration 12 = 勞動量；每拍進度 1（僅一人＋證人）或 2（再多一人幫忙）。
        params={"area": "海堤", "witnesses": 1, "rite": "焊水門", "not_before": 36,
                "duration": 12, "keep_witnesses": True,
                "too_early": "浪都還沒起，這時候把水門焊死？漁港就廢了，"
                             "外面的船一條也回不來。等潮訊確實了再說。"},
        uses=1,
    ),
    "lan_zha": ArtDef(
        ...
        name="攔閘",
        kind="social",
        tagline="站到閘口前面，用身體和嗓門把動工的人擋下來。",
        when="有人要焊水門、而你的好兄弟還在外海船上的時候。"
             "你擋得住一陣子，不是永遠——"
             "而且你很清楚：擋過頭，潮頭壓過來，低處的人會跟著賠上。"
             "這門功夫真正難的地方不是使出來，是決定什麼時候放手。",
        effect="block_rite",
        params={"rite": "焊水門", "ticks": 10, "area": "海堤",
                "line": "阿海橫在閘口前面，誰也動不了手。"},
        uses=2,
        cooldown=4,
    ),
    "wang_chao": ArtDef(
