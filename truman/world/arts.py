...
## 效果一覽（引擎怎麼解，見 Engine._resolve_art）
    ...
    block_rite  擋下某個儀式一段時間——反對者手上真正的籌碼
"""
...
CATALOG: dict[str, ArtDef] = {
    ...
    "zhen_chao_li": ArtDef(
        ...
        name="做海醮",
        kind="social",
        tagline="在鎮廟當眾起香做醮：至少請到兩名你看得到的觀禮者一起見證，讓海把暴潮壓下去。全庄的命懸在這一場。",
    ...
    "feng_zha": ArtDef(
        ...
        "什麼時候動手，是你今天最難的一個決定。這是苦力活，不是空話。",
        effect="rite",
        # not_before 36 對上導演腳本的第三則潮訊（「外海一道白線推近，
        # 沒回港的船看不見影」）。t3／t4 兩場水閘第 8／15 拍就焊完，
        # 導演鋪的整串潮訊全被跳過，72 拍的期限形同不存在。
        params={"area": "海堤", "witnesses": 1, "rite": "焊水門", "not_before": 36,
                # 這句一定要能讓人「等得下去」。引擎會自動接上時辰——
                # 只寫「還不行」而不寫「要等到什麼時候」，人會撞十次牆然後放棄。
    ...
    "lan_zha": ArtDef(
        ...
        when="有人要焊水門、而你的船還沒回港的時候。你擋得住一陣子，不是永遠——"
